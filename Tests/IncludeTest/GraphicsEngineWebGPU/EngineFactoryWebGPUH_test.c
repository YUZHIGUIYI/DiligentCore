...
void TestEngineFactoryWebGPU_CInterface(void)
{
    ...
    struct IEngineFactoryWebGPU*  pFactory = GetEngineFactoryWebGPU();
    struct EngineWebGPUCreateInfo EngineCI = {0};
    IRenderDevice*                pDevice  = NULL;
    IDeviceContext*               pCtx     = NULL;
    IEngineFactoryWebGPU_CreateDeviceAndContextsWebGPU(pFactory, &EngineCI, &pDevice, &pCtx);

    struct SwapChainDesc      SCDesc           = {0};
    void*                     pNativeWndHandle = NULL;
    ISwapChain*               pSwapChain       = NULL;
    IEngineFactoryWebGPU_CreateSwapChainWebGPU(pFactory, pDevice, pCtx, &SCDesc, pNativeWndHandle, &pSwapChain);
}
