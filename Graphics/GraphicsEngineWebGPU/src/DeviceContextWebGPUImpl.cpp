...
{
    TDeviceContextBase::GetTileSize(TileSizeX, TileSizeY);
}
                                                ...
                                                RESOURCE_STATE_TRANSITION_MODE StateTransitionMode)
{
    ...
    if (m_wgpuRenderPassEncoder)
        ClearAttachment(RTIndex, COLOR_MASK_ALL, CLEAR_DEPTH_FLAG_NONE, RGBA, 0);
    else
        m_PendingClears.SetColor(RTIndex, RGBA);
}
                                           ...
                                           RESOURCE_STATE_TRANSITION_MODE StateTransitionMode)
{
    ...
    if (BuffDesc.Usage == USAGE_DEFAULT)
    {
        ...
        memcpy(DynAllocation.pData, pData, Size);
                                            ...
                                            RESOURCE_STATE_TRANSITION_MODE DstTextureStateTransitionMode)
{
    ...
    for (Uint32 LayerIdx = 0; LayerIdx < UpdateRegionDepth; ++LayerIdx)
    {
        for (Uint32 RawIdx = 0; RawIdx < CopyInfo.RowCount; ++RawIdx)
        {
            ...
            memcpy(DynAllocation.pData + DstOffset, static_cast<const uint8_t*>(SubresData.pData) + SrcOffset, CopyInfo.RowSize);
        }
    }
                                                    ...
                                                    MappedTextureSubresource& MappedData)
{
    ...
    if (TexDesc.Usage == USAGE_DYNAMIC)
    {
        ...
        const auto Iter = m_MappedTextures.emplace(MappedTextureKey{pTextureWebGPU->GetUniqueID(), MipLevel, ArraySlice}, MappedTexture{CopyInfo, std::move(DynAllocation)});
        if (!Iter.second)
...
void DeviceContextWebGPUImpl::Flush()
{
    ...
    for (auto& MemPage : m_SharedMemPages)
        wgpuQueueWriteBuffer(m_wgpuQueue, MemPage.wgpuBuffer.Get(), 0, MemPage.pData, MemPage.PageSize);
