...
{
    auto* pDXCompiler = WebGPUShaderCI.pDXCompiler;
    VERIFY_EXPR(pDXCompiler != nullptr && pDXCompiler->IsLoaded());
    std::vector<uint32_t> SPIRV;
    pDXCompiler->Compile(ShaderCI, ShaderCI.HLSLVersion, SPIRVDefine, nullptr, &SPIRV, ShaderCI.ppCompilerOutput);
                                           ...
                                           const ShaderWebGPUImpl::CreateInfo& WebGPUShaderCI)
{
    ...
    {
        SPIRV = GLSLangUtils::HLSLtoSPIRV(ShaderCI, GLSLangUtils::SpirvVersion::Vk100, SPIRVDefine, ShaderCI.ppCompilerOutput);
    }
    else
    {
        ...
        else
        {
            ...
            GLSLSourceString        = BuildGLSLSourceString(ShaderCI, WebGPUShaderCI.DeviceInfo, WebGPUShaderCI.AdapterInfo, TargetGLSLCompiler::glslang, SPIRVDefine);
    ...
    if (ShaderCI.Source != nullptr || ShaderCI.FilePath != nullptr)
    {
        ...
        switch (ShaderCompiler)
        {
            case SHADER_COMPILER_DXC:
                m_SPIRV = CompileShaderDXC(ShaderCI, WebGPUShaderCI);
                break;
