...
// clang-format on
{
    ...
    else
    {
        if (m_Desc.Usage == USAGE_STAGING)
        {
            ...
            m_MappedData.resize(m_Desc.Size);
        }
        else
        {
        ...
        if (IsInitializeBuffer)
            wgpuQueueWriteBuffer(wgpuDeviceGetQueue(pDevice->GetWebGPUDevice()), m_wgpuBuffer.Get(), 0, pInitData->pData, pInitData->DataSize);
    }
...
IMPLEMENT_QUERY_INTERFACE(BufferWebGPUImpl, IID_BufferWebGPU, TBufferBase)
    ...
    if (MapType == MAP_READ)
    {
        auto MapAsyncCallback = [](WGPUBufferMapAsyncStatus MapStatus, void* pUserData) {
            if (MapStatus == WGPUBufferMapAsyncStatus_Success)
            {
                ...
                const auto* pData = static_cast<const uint8_t*>(wgpuBufferGetConstMappedRange(pThis->m_wgpuBuffer.Get(), 0, pThis->m_Desc.Size));
                VERIFY_EXPR(pUserData != nullptr);
                memcpy(pThis->m_MappedData.data(), pData, pThis->m_Desc.Size);
        ...
        wgpuBufferMapAsync(m_wgpuBuffer.Get(), WGPUMapMode_Read, 0, m_Desc.Size, MapAsyncCallback, this);
...
void BufferWebGPUImpl::Unmap(MAP_TYPE MapType)
{
    ...
    else if (MapType == MAP_WRITE)
    {
        auto MapAsyncCallback = [](WGPUBufferMapAsyncStatus MapStatus, void* pUserData) {
            ...
            {
                auto* pThis = static_cast<BufferWebGPUImpl*>(pUserData);
                auto* pData = static_cast<uint8_t*>(wgpuBufferGetMappedRange(pThis->m_wgpuBuffer.Get(), 0, pThis->m_Desc.Size));
                VERIFY_EXPR(pUserData != nullptr);
                memcpy(pData, pThis->m_MappedData.data(), pThis->m_Desc.Size);
                wgpuBufferUnmap(pThis->m_wgpuBuffer.Get());
            }
            else
            {
        ...
        wgpuBufferMapAsync(m_wgpuBuffer.Get(), WGPUMapMode_Write, 0, m_Desc.Size, MapAsyncCallback, this);
#ifndef PLATFORM_EMSCRIPTEN
        wgpuQueueSubmit(wgpuDeviceGetQueue(m_pDevice->GetWebGPUDevice()), 0, nullptr);
#endif
    }
...
void BufferWebGPUImpl::CreateViewInternal(
